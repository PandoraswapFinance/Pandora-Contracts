...
// File contracts/ProxyMeerkat.sol
...
contract ProxyPandora is OwnableUpgradeable, WhitelistUpgradeable {
    ...
    IBEP20 private constant PANDORA = IBEP20(0x5Cf5925602000cE4D3A88dA482Afc25b9C063493);
    ...
    function safeMeerkatTransfer(address to, uint256 amount) external onlyWhitelisted returns (uint256) {
        uint256 meerkatBal = PANDORA.balanceOf(address(this));
        if (amount > meerkatBal) {
            PANDORA.transfer(to, meerkatBal);
            return meerkatBal;
        } else {
